...
class GaussianModel:
    ...
    def sort_into_grid(self, compression_cfg, plas_config: DictConfig, verbose=False):
        ...
        _, sorted_indices = sort_with_plas(grid_to_sort, **plas_config) # type: ignore
    ...
    def as_grid_img(self, tensor):
        if not hasattr(self, "grid_sidelen"):
            raise ValueError("Gaussians not pruned yet!")

        if self.grid_sidelen * self.grid_sidelen != tensor.shape[0]:
            raise ValueError("Tensor shape does not match img sidelen, needs pruning?")
    ...
    def set_attr_from_grid_img(self, attr_name, img):
        ...
        if self.optimizer is not None:
            raise ValueError("Overwriting Gaussians during training not implemented yet! - Consider pruning method implementations")
