...
///////////////////////////////////////////////////////////////
// BoxFilterViaNPPI implementation (apply function and helpers)
///////////////////////////////////////////////////////////////

nvcv::Tensor
wrap_as_nvcv_tensor(float* d_data, int width, int height, int channels)
{
  nvcv::TensorDataStridedCuda::Buffer inBuffer;
  inBuffer.basePtr = reinterpret_cast<NVCVByte*>(d_data);
  inBuffer.strides[3] = sizeof(float);
  inBuffer.strides[2] = channels * inBuffer.strides[3];
  inBuffer.strides[1] = width * inBuffer.strides[2];
  inBuffer.strides[0] = height * inBuffer.strides[1];
  ...
  nvcv::Tensor::Requirements reqs =
    nvcv::Tensor::CalcRequirements(1, { width, height }, fmt);
...
void
GaussianFilterViaCVCUDA::apply(const float* d_input,
                               float* d_output,
                               int kernel_size)
{
  ...
  for (const auto& g : groups_) {
    // Gather the group channels into contiguous memory
    gather_channels_kernel<<<blocks, threads>>>(d_input,
                                                d_gather_buffer_,
                                                num_pixels,
                                                channels_,
                                                g.start_channel,
    ...
    nvcv::Tensor inTensor =
      wrap_as_nvcv_tensor(d_gather_buffer_ + g.start_channel * num_pixels,
                          width_,
                          height_,
                          g.group_size);
    nvcv::Tensor outTensor =
      wrap_as_nvcv_tensor(d_filter_buffer_ + g.start_channel * num_pixels,
                          width_,
                          height_,
                          g.group_size);

    cvcudaGaussianSubmit(op_,
                         stream_,
                         inTensor.handle(),
                         outTensor.handle(),
                         kernel_size,
    ...
    // Scatter the filtered group data back to the final interleaved output
    scatter_channels_kernel<<<blocks, threads>>>(d_filter_buffer_,
                                                 d_output,
                                                 num_pixels,
                                                 channels_,
                                                 g.start_channel,
...
// Computes the grouping of channels (each group size is one of {4, 3, 1})
// that minimizes the number of groups.
std::vector<int>
GaussianFilterViaCVCUDA::compute_groups(int C)
{
  std::vector<int> num_groups(C + 1, C + 1);
  std::vector<int> best_choice(C + 1, -1);
  num_groups[0] = 0;
  for (int i = 1; i <= C; i++) {
    for (int choice : { 1, 3, 4 }) {
      if (i - choice >= 0) {
        int num_groups_choice = num_groups[i - choice] + 1;
        if (num_groups_choice < num_groups[i]) {
          num_groups[i] = num_groups_choice;
  ...
  int remaining = C;
  while (remaining > 0) {
    groups.push_back(best_choice[remaining]);
    remaining -= best_choice[remaining];
  }
  return groups;
}
...
void
GaussianFilterViaCVCUDA::init_groups()
{
  groups_.clear();
  int current_channel = 0;
  std::vector<int> group_sizes = compute_groups(channels_);
  for (int gs : group_sizes) {
    Group g;
    g.start_channel = current_channel;
    g.group_size = gs;
    groups_.push_back(g);
    current_channel += gs;
  }
...
// Constructor: allocate GPU buffers and initialize groups
GaussianFilterViaCVCUDA::GaussianFilterViaCVCUDA(int width,
                                                 int height,
                                                 int channels)
  : width_(width)
  , height_(height)
  , channels_(channels)
  , d_gather_buffer_(nullptr)
  , d_filter_buffer_(nullptr)
{
  size_t total = static_cast<size_t>(width_) * height_ * channels_;
  ;
  cudaError_t err = cudaMalloc(&d_gather_buffer_, total * sizeof(float));
  assert(err == cudaSuccess);
  err = cudaMalloc(&d_filter_buffer_, total * sizeof(float));
  assert(err == cudaSuccess);
  init_groups();
  cudaStreamCreate(&stream_);
  NVCVStatus status = cvcudaGaussianCreate(&op_, width_, height_, 1);
  assert(status == NVCV_SUCCESS);
}

// Destructor: free the allocated GPU buffers
GaussianFilterViaCVCUDA::~GaussianFilterViaCVCUDA()
{
  if (d_gather_buffer_)
    cudaFree(d_gather_buffer_);
  if (d_filter_buffer_)
    cudaFree(d_filter_buffer_);
}